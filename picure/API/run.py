...
def get_run():
    runs = (
        get_db()
        .cursor()
        .execute("SELECT pr.id as id, pr.program_id as program_id, pr.enabled as enabled, pr.start_timestamp as start_timestamp, p.name as name FROM program_run pr join program p on pr.program_id = p.id")
        .fetchall()
    )
    return json.dumps(
        [
...
def add_run():
    ...
    get_db().cursor().execute("INSERT INTO program_run (program_id, enabled, start_timestamp ) values (?,?, (SELECT strftime('%s','now')) )",
