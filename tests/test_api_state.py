...
def test_state_control_get(client):
    ...
    assert client.get("/state/HEATING,UV").data == b'[["HEATING", false], ["UV", false]]'
...
def test_error_on_sensor_set(client):
    assert client.post("/state/SENSOR_TEMP", data=dict(state="Off")).status == '500 INTERNAL SERVER ERROR'
    assert client.post("/state/SENSOR_TEMP,SENSOR_HUMID", data=dict(state="Off")).status == '500 INTERNAL SERVER ERROR'
...
def test_state_control_sanity_checking(client):
    assert client.post("/state/HEATING", data=dict(switch=1, state="On")).status == '500 INTERNAL SERVER ERROR'
    assert client.post("/state/HEATING", data=dict(switch=0, state="Off")).status == '500 INTERNAL SERVER ERROR'